...
contract Crowdfunding{
    ...
    function refund() public {
        ...
        backers[msg.sender].totalContribution = 0; //Re-entrancy guard
        payable(msg.sender).transfer(amount);
    }

    function hasFundendTier(address _backer, uint256 _tierIndex) public view returns(bool){
    ...
    function getCampaignStatus() public view returns(CampaignState){
        if(state == CampaignState.Active && block.timestamp > deadline){
            address(this).balance >= goal ? CampaignState.Successful : CampaignState.Failed;
        }
        return state;
    }
